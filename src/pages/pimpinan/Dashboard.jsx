...
const DashboardPimpinanPage = () => {
  ...
  const pdfSectionStatistikRef = useRef(null);
  const pdfSectionGrafikRef = useRef(null);
  const pdfSectionPanenRef = useRef(null);
  ...
  const kecamatanChartOptions = {
    chart: {
      ...
      backgroundColor: "transparent",
      marginRight: 30, // Memberi ruang untuk label angka di kanan
    },
    title: { text: null },
    credits: { enabled: false },
    ...
    series: [
      {
        ...
        data: [
          { y: 90, color: "#064E3B", label: "12K" }, // Sumbersari (Hijau Sangat Tua)
...
<path d="M6.6665 6.36182V1.33353H1.63822" stroke="#22C55E" stroke-width="0.8" stroke-linecap="round" stroke-linejoin="round"/>
  ...
  const captureSectionToPdfPage = async (element, pdf, isFirstPage) => {
    const canvas = await html2canvas(element, {
      ...
      onclone: (clonedDoc, clonedRoot) => {
        clonedDoc.querySelectorAll(".pdf-export-hide").forEach((node) => {
        ...
        if (clonedRoot) {
          ...
          if (w > 0) {
            ...
            clonedRoot.style.maxWidth = `${w}px`;
    ...
    let imgHmm = availH;
    try {
      const dim = pdf.getImageProperties(imgData);
      if (dim?.width > 0 && dim?.height > 0) {
        const ratio = Math.min(availW / dim.width, availH / dim.height);
        imgWmm = dim.width * ratio;
        imgHmm = dim.height * ratio;
      }
    } catch {
      ...
      if (imgAspect > availAspect) {
        imgWmm = availW;
        imgHmm = availW / imgAspect;
      } else {
        imgHmm = availH;
        imgWmm = availH * imgAspect;
      }
    }

    const x = margin + (availW - imgWmm) / 2;
    const y = margin + (availH - imgHmm) / 2;
  ...
  const handleExportPdf = async () => {
    ...
    if (!s1 || !s2 || !s3 || isPdfExporting) return;
    ...
    try {
      ...
      for (let i = 0; i < sections.length; i += 1) {
        ...
        await captureSectionToPdfPage(sections[i], pdf, i === 0);
      }

      pdf.save(`dashboard-sipadu-${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (err) {
      console.error(err);
      alert("Gagal mengekspor PDF. Tutup modal lain lalu coba lagi.");
    } finally {
      window.scrollTo({
        top: prevScroll.y,
        left: prevScroll.x,
  ...
  return (
    ...
    <div className="w-full px-[44px] py-10 space-y-10 bg-[#F9FAFB] min-h-screen">
      <div ref={pdfSectionStatistikRef} className="space-y-10">
      ...
      <div className="flex flex-row justify-between">
        ...
        <div className="flex pdf-export-hide">
          <div className="flex items-center w-full">
            <button
              type="button"
              disabled={isPdfExporting}
              onClick={handleExportPdf}
              className="mr-4 px-4 py-2.5 bg-black text-white text-[14px] font-bold hover:bg-gray-800 transition-colors disabled:opacity-60"
            >
              {isPdfExporting ? "Menyiapkan PDF…" : "Export"}
            </button>
            <button
      ...
      <div ref={pdfSectionGrafikRef} className="space-y-10">
      {/* Grafik Komoditas */}
      ...
      <div ref={pdfSectionPanenRef} className="space-y-10">
...
export default DashboardPimpinanPage;